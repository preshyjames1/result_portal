...
export default function SchoolAdminDashboardPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [studentsRes, pinsRes, resultsRes] = await Promise.all([
          fetch('/api/admin/students?limit=1'),
          fetch('/api/admin/pins?limit=1'),
          fetch('/api/admin/results'),
        ]);
        const [studentsData, pinsData, resultsData] = await Promise.all([
          studentsRes.json(),
          pinsRes.json(),
          resultsRes.json(),
        ]);
        ...
        const pinsUsedToday = (pinsData.pins ?? []).filter((p: { usage_count: number; created_at: string }) =>
          p.usage_count > 0 && new Date(p.created_at).toDateString() === today
        ).length;
    ...
    <div>
      ...
      <div className="mt-8 bg-white rounded-lg border border-gray-200 p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {[
            { href: '/school-admin/students', label: 'Add Student', icon: '➕' },
            { href: '/school-admin/results', label: 'Upload Result', icon: '📤' },
            { href: '/school-admin/pins', label: 'Buy PINs', icon: '🛒' },
          ].map((action) => (
